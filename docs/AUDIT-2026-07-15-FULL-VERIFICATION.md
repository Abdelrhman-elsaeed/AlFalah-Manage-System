...
| GAP 4 — P10 warning and guard regressions | **Completed 2026-07-15** | `ImprovementPlanService` nullable EF navigation is fixed and the full solution builds at 0 warnings/errors. Real public-service regression tests prove D-36 blocks Instructor use of supervisor detail and D-75 blocks both pure Moderator and Moderator+SchoolManager complaint access before data access. 70/70 backend tests pass; frontend build remains green; ar/en parity remains 461/461. |
| GAP 5 — Whole-app polish | **Completed 2026-07-15** | All application selects converge on the unified clearable-select and all dates use `p-calendar`; plan/follow-up screens and the broader audited UI use ar/en i18n and Saudi-theme controls. D-70/D-71 dialog-close, reset, and aggregate-refresh paths are verified. Static scans find no native selects/date inputs or direct feature dropdowns. Backend build remains 0 warnings/errors; 70/70 tests pass; frontend build is green; ar/en parity is 623/623 with no duplicate top-level or missing literal keys. |
