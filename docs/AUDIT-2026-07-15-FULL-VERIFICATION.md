...
| P5 approval / visibility (D-36/D-37) | `PHASE-05-*` | PARTIAL — approval state machine and `ReportViewLog` exist; `GetByIdAsync` returns an instructor-owned non-approved payload and generic endpoints retain instructor access | PARTIAL — dedicated `/instructor/reports` exists and sidebar hides supervisor tabs | `Phase5ApprovalVisibility` | **P1 security:** instructor must receive 403 from supervisor list/detail/analysis/PDF endpoints; only `/my-approved-reports` and approved report endpoint may be used. |
...
| D-72 teachers / profile / auto-fill | `14-DECISIONS-*` | EXISTS — teacher profile/classes, teaching endpoints and scoped visit support | EXISTS — list/profile/form auto-fill integration | `AddTeacherProfileClasses` | Validate a background 403 does not navigate away (interceptor supports suppression; call-site confirmation pending). |
...
1. **Security:** prohibit instructor use of supervisor visit list/detail/analysis/PDF/export endpoints; leave only own approved report feed and report endpoint (D-36).
2. **Security:** keep Moderator complaints hard-blocked across service, routes, navigation and dashboard/export payloads (D-75); remove stale UI assumptions.
3. **Functional:** provide instructor complaint submission from an approved report after a recorded view.
4. **Functional:** replace all four dashboard placeholders with scoped role dashboards and Excel/PDF exports.
5. **Functional/UI:** rebuild the D-73 categorized sidebar and complete the visibility-specific navigation matrix.
6. **Quality/UI:** remove the nullable warning, verify D-70/D-71 interaction behaviour, normalize remaining controls and move hard-coded UI text into i18n with ar/en parity.
7. **Deployment verification:** apply/inspect migrations only against the configured development database; do not alter production.
